...
public class ATM_Machine {
	...
	//int validcard = 0;
	String tocheckcount;	// to check count of invalid pin( retrieve string)
	int Receipt_no;			// will get the max value of receipt number
	int softlock;			// will get value of softlock from the database for that account
	String temppin;			// will store the encrypted value of pin
	String tempcardnum;		// will store the encrypted value of card
	String StringAccount_no;// will store account number
	Integer Account_no;		// will convert the StringAccount_no to integer(used for passing account number as integer) 
	final int countvalue = 3;// used for calculating "chances left"
	...
	Integer count = 0; // counts number of time user has input invalid entries(int)

	public ATM_Machine(int acntno, int rcpt) {		//parameterized constructor
	...
	public static boolean validateJavaDate(String strDate) {				//to check is the card is expired or not
...
//	}
	public void Validation() {								//will validate card details(cardnumber and respective pin) , calls  validateJavaDate for validating expiry date, checks if a card is blocked and implements softlock
		...
		try {
			...
			Card_number = myObj.nextLine();					//takes card number from console
			System.out.println("--------------------------------------");
			System.out.print("\nEnter pin:  ");
			pin = myObj.nextLine();							//takes pin from console
			logger.info("User Entered Pin");
			System.out.println("--------------------------------------\n");

			// new code
			String sql11 = "Select pkg_app_security.fn_encrypt_value ('" + Card_number + "') rh from dual";		// query for encrypting card number
			String sql12 = "Select pkg_app_security.fn_encrypt_value ('" + pin + "') rh from dual";				// query for encrypting pin 
			...
			m.next();
			if (tempcardnum.equals(m.getString(2)) && temppin.equals(m.getString(4)))// validation of pin wrt the entered card number
			{
				...
				if (validateJavaDate(n.getString(5))) {				// expiry date verification
					if (n.getString(6).equals("1")) {				// checks if a card is blocked or not
						String sql5 = "update card_pin set INVALID_ENTRIES =" + 0 + " where CARD_NO="
								+ this.Card_number;
						...
						if (this.softlock == 1) {				//implements softlock
				...
				if (Card_number.equals(m.getString(1))) {		//invalid pin tries
					int temp;
					...
					if (this.count < 3) {
						...
						String sql3 = "update card_pin set INVALID_ENTRIES =" + this.count + " where CARD_NO="
								+ this.Card_number;
		...
		catch (Exception x) {					// if a card is invalid
	...
	public static void main(String[] jjjjjj) {				//main function
